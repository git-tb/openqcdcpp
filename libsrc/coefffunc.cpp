...
{
	double result(0.);
	result += QCD::CF * (-2. * (1. + z) * std::log(1. - z) - 2. * (1. + z * z) / (1. - z) * std::log(z) + 6. + 4. * z);
	return result;
}
...
{
	double result(0.);
	result += QCD::TR * 4 * (+(1. - 2. * z + 2. * z * z) * std::log((1. - z) / z) - 1 + 8. * z * (1. - z));
	return result;
}
...
/// @brief from [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005] eq. 4.8
...
/// @brief eq. (B.5) in [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005]
...
double c2q_ns_2_01_local_exact(int nf)
{
	...
	result += std::pow(
				  QCD::CF, 2) *
				  (331. / 8. + 69 * MATH::ZETA2 + 6 * std::pow(MATH::ZETA2, 2) -
				   78 * MATH::ZETA3) +
			  QCD::CA *
...
double c2q_ns_2_0_localplus_approx(double z)
{
	...
	result += 14.222 * std::pow(L1, 4) / 4.0 - 61.3333 * std::pow(L1, 3) / 3.0 - 31.105 * std::pow(L1, 2) / 2.0 + 188.64 * L1;
	return result;
}
...
double c2q_ns_2_1_localplus_approx(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += 1.77778 * std::pow(L1, 3) / 3.0 - 8.5926 * std::pow(L1, 2) / 2.0 + 6.3489 * L1;
	return result;
}
...
double c2q_ns_2_0_localplus_approx2(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += 128. / 9. * std::pow(L1, 4) / 4. - 184. / 3. * std::pow(L1, 3) / 3. - 31.1052 * std::pow(L1, 2) / 2. + 188.64 * L1;
	return result;
}
...
double c2q_ns_2_1_localplus_approx2(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += 16. / 9. * std::pow(L1, 3) / 3. - 232. / 27. * std::pow(L1, 2) / 2. + 6.34888 * L1;
	return result;
}
...
double c2q_ns_2_01_localplus_exact(double x, int nf)
{
	...
	result += QCD::CA *
			  ...
			  QCD::CF *
				  nf * ((247. / 27. + -8. / 3. * MATH::ZETA2) * L1x + -29. / 9. * std::pow(L1x, 2) + 4. / 9. * std::pow(L1x, 3)) +
			  std::pow(
				  QCD::CF, 2) *
				  ((51. / 2. + 36 * MATH::ZETA2 - 8 * MATH::ZETA3) * L1x + (-27. / 2. - 16 * MATH::ZETA2) * std::pow(L1x, 2) - 6 * std::pow(L1x, 3) +
				   2 * std::pow(L1x, 4));
	return result;
}
...
double c2q_ns_2_0_plus_approx(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += (14.2222 * std::pow(L1, 3) - 61.3333 * std::pow(L1, 2) - 31.105 * L1 + 188.64) / (1.0 - z);
	return result;
}
...
double c2q_ns_2_1_plus_approx(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += (1.77778 * std::pow(L1, 2) - 8.5926 * L1 + 6.3489) / (1.0 - z);
	return result;
}
...
double c2q_ns_2_0_plus_approx2(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += (128. / 9. * std::pow(L1, 3) - 184. / 3. * std::pow(L1, 2) - 31.1052 * L1 + 188.641) / (1.0 - z);
	return result;
}
...
double c2q_ns_2_1_plus_approx2(double z)
{
	double result(0.0);
	double L1 = std::log(1 - z);
	result += (16. / 9. * std::pow(L1, 2) - 232. / 27. * L1 + 6.34888) / (1.0 - z);
	return result;
}
...
double c2q_ns_2_01_plus_exact(double z, int nf)
{
	...
	result += (QCD::CA *
			   ...
			   QCD::CF *
				   nf * (-247. / 27. + 8. / 3. * MATH::ZETA2 + 58. / 9. * L1z + -4. / 3. * std::pow(L1z, 2)) +
			   std::pow(
				   QCD::CF, 2) *
				   (-51. / 2. - 36 * MATH::ZETA2 + 8 * MATH::ZETA3 + (27 + 32 * MATH::ZETA2) * L1z + 18 * std::pow(L1z, 2) - 8 * std::pow(L1z, 3))) /
			  (-1 +
			   z);
	return result;
}
...
double c2q_ns_2_0_reg_approx(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += -17.19 * std::pow(L1, 3) + 71.08 * std::pow(L1, 2) - 660.7 * L1 + L1 * L0 * (-174.8 * L1 + 95.09 * L0) - 2.835 * std::pow(L0, 3) - 17.08 * std::pow(L0, 2) + 5.986 * L0 - 1008 * z - 69.59;
	return result;
}
...
double c2q_ns_2_1_reg_approx(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += -1.707 * std::pow(L1, 2) + 22.95 * L1 + L1 * L0 * (3.036 * L0 + 17.97) + 2.244 * std::pow(L0, 2) + 5.770 * L0 - 37.91 * z - 5.691;
	return result;
}
...
double c2q_ns_2_0_reg_approx2(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += -17.74 * std::pow(L1, 3) + 72.24 * std::pow(L1, 2) - 628.8 * L1 - 181.0 - 806.7 * z + 0.719 * z * std::pow(L0, 4) + L1 * L0 * (37.75 * L0 - 147.1 * L1) - 28.384 * L0 - 20.70 * std::pow(L0, 2) - 80. / 27. * std::pow(L0, 3);
	return result;
}
...
double c2q_ns_2_1_reg_approx2(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += -1.500 * std::pow(L1, 2) + 24.87 * L1 - 7.8109 - 17.82 * z - 12.97 * std::pow(z, 2) - 0.185 * z * std::pow(L0, 3) + 8.113 * L0 * L1 + 16. / 3. * L0 + 20. / 9. * std::pow(L0, 2);
	return result;
}
...
double c2q_ns_2_01_reg_exact(double z, int nf)
{
	...
	result += QCD::CA *
			  ...
			  QCD::CF *
				  nf * (-158. / 27. + -488. / 27. * z + (8. / 3. + 8. / 3. * z) * MATH::ZETA2 - (4 * HPL1_0z) / (-1 + std::pow(z, 2)) + (-26. / 3. * std::pow(z, 2) * HPL1_0z) / (-1 + std::pow(z, 2)) + (-38. / 3. * std::pow(z, 3) * HPL1_0z) / (-1 + std::pow(z, 2)) + -32. / 9. * HPL1_1z + -68. / 9. * z * HPL1_1z + -10. / 3. * HPL2_00z + -10. / 3. * z * HPL2_00z + -8. / 3. * HPL2_01z + -8. / 3. * z * HPL2_01z + -4. / 3. * HPL2_10z + -4. / 3. * z * HPL2_10z + -4. / 3. * HPL2_11z + -4. / 3. * z * HPL2_11z + (8. / 3. * MATH::ZETA2 + -58. / 9. * HPL1_1z + -20. / 3. * HPL2_00z + -16. / 3. * HPL2_01z + -8. / 3. * HPL2_10z + -8. / 3. * HPL2_11z + -58. / 9. * L1z + 4. / 3. * std::pow(L1z, 2)) / (-1 + z)) +
			  std::pow(
				  QCD::CF, 2) *
				  (-124. / 5. + 16. / 5. / z + -461. / 5. * z + 144. / 5. * std::pow(z, 2) +
				   (-64 + 72 * z) * MATH::ZETA3 + (-93. / 5. * HPL1_0z) / (-1 + std::pow(z, 2)) +
				   (16. / 5. * HPL1_0z) / (z * (-1 + std::pow(z, 2))) + (101. / 5. * z * HPL1_0z) / (-1 + std::pow(z, 2)) + (-276. / 5. * std::pow(z, 2) * HPL1_0z) / (-1 + std::pow(z, 2)) + (-502. / 5. * std::pow(z, 3) * HPL1_0z) / (-1 + std::pow(z, 2)) +
				   (144. / 5. * std::pow(z, 4) * HPL1_0z) / (-1 + std::pow(z, 2)) + 16 * HPL1_1z -
				   68 * z * HPL1_1z + MATH::ZETA2 * (-32 - 8 * z + -144. / 5. * std::pow(z, 3) - (24 * HPL1_0z) / (-1 + std::pow(z, 2)) - (8 * z * HPL1_0z) / (-1 + std::pow(z, 2)) - (40 * std::pow(z, 2) * HPL1_0z) / (-1 + std::pow(z, 2)) - (24 * std::pow(z, 3) * HPL1_0z) / (-1 + std::pow(z, 2)) - 16 * HPL1_1z + 32 * z * HPL1_1z) + 72 * HPL2_m10z + (16. / 5. * HPL2_m10z) / std::pow(z, 2) +
				   ...
				   30 * HPL3_000z + 6 * z * HPL3_000z + (56 * MATH::ZETA3 + MATH::ZETA2 * (-40 * HPL1_m1z - 48 * z * HPL1_m1z - 72 * std::pow(z, 2) * HPL1_m1z) - 64 * HPL3_m1m10z + 80 * HPL3_m100z + 32 * HPL3_m101z + 48 * HPL3_0m10z - 24 * HPL3_000z - 16 * HPL3_001z) / (1 + z) + 40 * HPL3_001z + 24 * z * HPL3_001z +
				   ...
				   (-72 * MATH::ZETA3 - 27 * HPL1_1z + 6 * HPL2_00z + 24 * HPL2_01z +
					...
					48 * HPL3_101z + 64 * HPL3_110z + 48 * HPL3_111z + MATH::ZETA2 * (12 - 16 * HPL1_1z - 32 * L1z) - 27 * L1z - 18 * std::pow(L1z, 2) +
					8 * std::pow(L1z, 3)) /
					   (-1 + z));
	return result;
}
...
double c2g_2_0_reg_approx(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += (6.445 + 209.4 * (1 - z)) * std::pow(L1, 3) - 24.00 * std::pow(L1, 2) + (1494.0 / z - 1483) * L1 + L1 * L0 * (-871.8 * L1 - 724.1 * L0) + 5.319 * std::pow(L0, 3) - 59.48 * std::pow(L0, 2) - 284.8 * L0 + 11.90 / z + 392.4;
	return result;
}

/// @brief from [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005] eq. 4.10
double c2g_2_0_reg_approx2(double z)
{
	double result(0.0);
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += (58. / 9. * std::pow(L1, 3) - 24 * std::pow(L1, 2) - 34.88 * L1 + 30.586 - (25.08 + 760.3 * z + 29.65 * std::pow(L1, 3)) * (1 - z) + 1204 * z * std::pow(L0, 2) + L0 * L1 * (293.8 + 711.2 * z + 1043 * L0) + 115.6 * L0 - 7.109 * std::pow(L0, 2) + 70. / 9. * std::pow(L0, 3) + 11.9033 * (1. - z) / z);
	return result;
}

/// @brief eq. (B.6) in [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005]
/// This expression was obtained after some rewriting and reordering with Mathematica.
double c2g_2_0_reg_exact(double z)
{
	...
	result += QCD::CF * (-647. / 15. + 8. / 15. / z + 239. / 5. * z + -36. / 5. * std::pow(z, 2) +
						 ...
						 24 * std::pow(z, 2) * HPL1_1z + MATH::ZETA2 * (16 + -104. / 3. * z + 72 * std::pow(z, 2) + 96. / 5. * std::pow(z, 3) - 16 * HPL1_m1z - 32 * z * HPL1_m1z - 16 * std::pow(z, 2) * HPL1_m1z + 16 * HPL1_0z - 32 * z * HPL1_0z + 48 * std::pow(z, 2) * HPL1_0z + 8 * HPL1_1z - 16 * z * HPL1_1z + 32 * std::pow(z, 2) * HPL1_1z) + 48 * HPL2_m10z +
						 ...
						 16 * HPL2_01z + 56 * z * HPL2_01z - 72 * std::pow(z, 2) * HPL2_01z - 26 * HPL2_10z + 80 * z * HPL2_10z - 72 * std::pow(z, 2) * HPL2_10z - 26 * HPL2_11z +
			  ...
			  QCD::CA * (239. / 9. + 344. / 27. / z + 1072. / 9. * z + -4493. / 27. * std::pow(z, 2) + (4 - 48 * z + 24 * std::pow(z, 2)) * MATH::ZETA3 + 58 * HPL1_0z +
						 584. / 3. * z * HPL1_0z + -2090. / 9. * std::pow(z, 2) * HPL1_0z + 62. / 3. * HPL1_1z +
						 (-104. / 9. * HPL1_1z) / z + 454. / 3. * z * HPL1_1z +
						 -1570. / 9. * std::pow(z, 2) * HPL1_1z + MATH::ZETA2 * (8 + -16. / 3. / z - 144 * z + 148 * std::pow(z, 2) - 4 * HPL1_m1z - 8 * z * HPL1_m1z - 16 * std::pow(z, 2) * HPL1_m1z - 8 * HPL1_0z - 64 * z * HPL1_0z + 16 * std::pow(z, 2) * HPL1_0z + 8 * HPL1_1z - 16 * z * HPL1_1z + 8 * std::pow(z, 2) * HPL1_1z) - 24 * HPL2_m10z + (-16. / 3. * HPL2_m10z) / z +
						 ...
						 (16. / 3. * HPL2_11z) / z + 72 * z * HPL2_11z + -244. / 3. * std::pow(z, 2) * HPL2_11z + 8 * HPL3_m1m10z + 16 * z * HPL3_m1m10z + 8 * HPL3_m100z + 16 * z * HPL3_m100z +
...
/// @brief eq. (B.7) in [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005]
...
double c2q_ps_2_0_reg_approx(double z)
{
	...
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += -0.101 * (1 - z) * std::pow(L1, 3) - (24.75 - 13.80 * z) * L1 * std::pow(L0, 2) + 30.23 * L1 * L0 + 4.310 * std::pow(L0, 3) - 2.086 * std::pow(L0, 2) + 39.78 * L0 + 5.290 * (1.0 / z - 1);
	return result;
}

/// @brief from [Vermaseren, Vogt, Moch; Nucl. Phys. B, 724 (1-2), 3, 2005] eq. 4.9
double c2q_ps_2_0_reg_approx2(double z)
{
	double result(0.0);
	double L0 = std::log(z);
	double L1 = std::log(1 - z);
	result += (8. / 3. * std::pow(L1, 2) - 32. / 3. * L1 + 9.8937) * (1 - z) + (9.57 - 13.41 * z + 0.08 * std::pow(L1, 3)) * std::pow(1 - z, 2) + 5.667 * z * std::pow(L0, 3) - std::pow(L0, 2) * L1 * (20.26 - 33.93 * z) + 43.36 * (1 - z) * L0 - 1.053 * std::pow(L0, 2) + 40. / 9. * std::pow(L0, 3) + 5.2903 * std::pow(1 - z, 2) / z;
	return result;
}
...
double c2q_ps_2_0_reg_exact(double z)
{
	...
	result += QCD::CF * (158. / 9. + 344. / 27. / z + -422. / 9. * z + 448. / 27. * std::pow(z, 2) +
						 ...
						 -128. / 9. * std::pow(z, 2) * HPL1_0z + MATH::ZETA2 * (-16. / 3. / z - 16 * z + 16 * std::pow(z, 2) - 16 * HPL1_0z - 16 * z * HPL1_0z) + 104. / 3. * HPL1_1z +
