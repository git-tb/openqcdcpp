...
double F2(double x, double Q2)	{
	...
	if(QCDORDER::F2ORDER >= 1)	{
		...
		if(QCDORDER::F2ORDER >= 2)	{
			...
			result += QCD::NF * a4pi * a4pi * runcorr_ci_2 * c2q_ns_2_1_local_approx() * xfiQi2sum(x, Q2);
			result += a4pi * a4pi * runcorr_ci_2 * c2q_ns_2_0_localplus_approx(x) * xfiQi2sum(x, Q2);
			result += QCD::NF * a4pi * a4pi * runcorr_ci_2 * c2q_ns_2_1_localplus_approx(x) * xfiQi2sum(x, Q2);
...
double F2integrand(double z, double Q2, double x)	{
	...
	/// nlo
	if(QCDORDER::F2ORDER >= 1)	{
		result_ns_reg 	+= a4pi * runcorr_ci_1 * c2q_ns_1_0_reg(z) * xfiQi2sum(x/z, Q2);
		result_ns_plus 	+= a4pi * runcorr_ci_1 * c2q_ns_1_0_plus(z) * ( xfiQi2sum(x/z, Q2) - xfiQi2sum(x, Q2) );
